...
// Sequencer for the full 4-layer MLP run
// Runs the same single-layer engine four times with different layer settings
...
// Base addresses for weights, input, scratch buffers, and final output in BRAM
...
// Per-layer post-processor scale values in Q24 fixed-point format
...
// FSM that starts each layer and advances to the next settings when it finishes
...
// Next layer settings computed combinationally
...
// Latched settings passed into the layer engine
...
// Send a one-cycle start pulse to the single-layer engine
assign layer_start = en && (state == ST_START_LAYER);
// In ST_NEXT_LAYER, choose settings using the incremented layer index
assign load_layer_idx = (state == ST_NEXT_LAYER) ? (layer_idx + 1'b1) : layer_idx;

always_comb begin
    // Default to layer 0 settings
    ...
    case (load_layer_idx)
        2'd0: begin
            // layer 1: read the input spectrogram and store feature-major into SCRATCH0
            current_act_base = INPUT_BASE;
            current_wgt_base = W1_BASE;
            current_out_base  = SCRATCH0_BASE;
            current_wgt_out_stride = 14'd768; // 48 K tiles * 16 words
            // input_spectrogram.bin is pre-tiled row-major inside bram_init
            current_act_layout_row_major = 1'b1;
            current_out_layout_row_major = 1'b0;
            current_num_k_tiles = 8'd48;
            current_num_out_tiles = 8'd8;
        ...
        2'd1: begin
            // layer 2: read SCRATCH0 and store into SCRATCH1
        ...
        2'd2: begin
            // layer 3: read SCRATCH1 and store back into SCRATCH0
        ...
        2'd3: begin
            // layer 4: final result, stored row-major in the FINAL area
            current_act_base = SCRATCH0_BASE;
...
// The single_layer_engine handles the actual work for one layer
    ...
    if (rst || clear) begin
        ...
        case (state)
            ...
            ST_START_LAYER: begin
                // Raise one start pulse for the reused layer engine
                state <= ST_WAIT_LAYER_DONE;
            end

            ST_WAIT_LAYER_DONE: begin
                // Wait for the current layer to finish
                if (layer_done) begin
            ...
            ST_NEXT_LAYER: begin
                // Load the next layer settings into active registers and start again
            ...
            ST_DONE: begin
                // Raise done after all four layers finish
                done <= 1'b1;
                state <= ST_IDLE;
            end

            default: begin
                // Reset the sequencer state on an unexpected state
                state <= ST_IDLE;
                layer_idx <= '0;
                done <= 1'b0;
                active_act_base <= '0;
