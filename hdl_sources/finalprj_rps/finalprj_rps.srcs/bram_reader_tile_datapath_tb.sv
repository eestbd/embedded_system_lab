...
module bram_reader_tile_datapath_tb;
...
localparam int TIMEOUT_CYCLES = 240;
...
// Mock synchronous BRAM. The reader issues en/address on one clock, and this
// model updates rdata on the following clock edge by sampling the previous
// cycle's en/address values.
...
begin
    ...
    // Force a few easy waveform landmarks: positive, negative, zero, saturation.
...
begin
    for (int lane = 0; lane < N; lane++) begin
        ...
        if (reader_raw_act_vec[lane] !== expected_act) begin
            $display("READER_CHECK data valid_count %0d stream %0d lane %0d ACT expected=%0d got=%0d",
                     reader_valid_count, stream_idx, lane, expected_act, reader_raw_act_vec[lane]);
            $fatal(1);
        end
        if (reader_raw_wgt_vec[lane] !== expected_wgt) begin
            $display("READER_CHECK data valid_count %0d stream %0d lane %0d WGT expected=%0d got=%0d",
...
task automatic pulse_reader_start();
begin
    @(negedge clk);
    reader_start = 1'b1;
...
begin
    ...
    if (out_row_idx !== expected_idx) begin
        $display("TILE_OUTPUT_CHECK count %0d: expected row_idx=%0d, got %0d",
                 tile_out_count, expected_row, out_row_idx);
        $fatal(1);
    end
...
task automatic wait_for_tile_outputs();
    int tile_out_count;
    int timeout_count;
    bit tile_done_seen;
begin
    ...
    while (!tile_done_seen && (timeout_count < TIMEOUT_CYCLES)) begin
        @(posedge clk);
        #1;
        timeout_count = timeout_count + 1;
        ...
        if (tile_done) begin
            ...
            if (out_valid !== 1'b0) begin
                $display("TILE_OUTPUT_CHECK: expected out_valid=0 when tile_done=1, got %0b",
                         out_valid);
                $fatal(1);
            end
            if (tile_out_count != N) begin
    ...
    if (!tile_done_seen) begin
        $display("TILE_OUTPUT_CHECK: timeout waiting for tile_done");
        $fatal(1);
    end
    if (tile_out_count != N) begin
    ...
    @(posedge clk);
    #1;
    if (tile_done !== 1'b0) begin
        $display("TILE_OUTPUT_CHECK: expected tile_done pulse to clear, got %0b", tile_done);
        $fatal(1);
    end
    if (out_valid !== 1'b0) begin
        $display("TILE_OUTPUT_CHECK: expected out_valid=0 after tile_done, got %0b", out_valid);
        $fatal(1);
    end
end
endtask
...
initial begin
    ...
    wait_for_tile_outputs();

    $display("BRAM_READER_TILE_DATAPATH test PASSED");
    repeat (4) @(posedge clk);
    $finish;
end
