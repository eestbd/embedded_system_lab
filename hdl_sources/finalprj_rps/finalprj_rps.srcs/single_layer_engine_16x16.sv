module single_layer_engine_16x16 #(
    parameter int N           = 16,
...
output_tile_engine_16x16 #(
    ...
    .start         (tile_start),
    .en            (en),
    // The same activation tile group is reused for every output column tile.
    .act_base_addr (act_base_addr),
    // Weight and output bases advance once per output column tile.
    ...
    if (rst || clear) begin
        ...
        case (state)
            ST_IDLE: begin
                ...
                if (start) begin
                    // Start with output columns 0..15.
                    state <= ST_START_TILE;
                end
            end

            ST_START_TILE: begin
                // One-cycle start pulse for output_tile_engine_16x16.
            ...
            ST_NEXT_OUT_TILE: begin
                // Move to the next output column tile. This is equivalent to:
                // wgt_base_addr + out_tile_idx*wgt_out_stride
                // out_base_addr + out_tile_idx*out_tile_stride
