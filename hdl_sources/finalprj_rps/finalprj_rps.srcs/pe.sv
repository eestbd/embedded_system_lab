...
// Single PE cell
// Passes act to the right and weight downward while accumulating the local product
module PE #(
...
// Product width starts at twice DATA_W since two DATA_W values are multiplied
localparam int PRODUCT_W = DATA_W * 2;

logic signed [PRODUCT_W-1:0] product;
logic signed [ACC_W-1:0] product_ext;

// Product of the activation and weight currently entering this PE
assign product = $signed(i_act) * $signed(i_wgt);

// Match the product to the accumulator width, sign-extending when needed
generate
...
// Values move one PE and accumulate only on enabled cycles
always_ff @(posedge i_clk or negedge i_rst_n) begin
    if (!i_rst_n) begin
        ...
        if (i_clear) begin
            // Clear only the accumulated sum when starting a new calculation
            o_acc <= '0;
        end
        else if (i_en) begin
            // Add the local product to the current partial sum
            o_acc <= o_acc + product_ext;
        end

        if (i_en) begin
            // act and weight are pipelined into the next PE
            o_act <= i_act;
            o_wgt <= i_wgt;
        end
    end
